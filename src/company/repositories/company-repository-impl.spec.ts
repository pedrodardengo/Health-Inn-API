...
describe('CompanyRepositoryImpl Integration test', () => {
    ...
    beforeEach(async () => {
        ...
        workRelationRepo = await testingModule.get(getRepositoryToken(WorkRelation))
    })
    ...
    describe('inactivateAllWorkRelationsOfEmployee', () => {
        ...
        it('should deactivate any active work relation of a given employee if any', async () => {
            // Arrange
            const employee = await employeeRepo.save(new Employee().build(new EmployeeExampleBuilder().employee))

            const company1 = await companyRepoImpl.create(generateRandomCompanyExample())
            const workRelationData1 = generateWorkRelationExample(employee.cpf, company1.cnpj, true)
            const workRelation1 = new WorkRelation().build(employee, company1, workRelationData1)
            await companyRepoImpl.createWorkRelation(workRelation1)

            const company2 = await companyRepoImpl.create(generateRandomCompanyExample())
            const workRelationData2 = generateWorkRelationExample(employee.cpf, company2.cnpj, false)
            const workRelation2 = new WorkRelation().build(employee, company2, workRelationData2)
            await companyRepoImpl.createWorkRelation(workRelation2)

            // Act
            await companyRepoImpl.inactivateAllWorkRelationsOfEmployee(employee)
