...
export class CompanyRepositoryImpl implements CompanyRepository {
    constructor(
        ...
        @InjectRepository(
            WorkRelation) private workRelationRepo: Repository<WorkRelation>,
    ) {
    }
    ...
    async inactivateAllWorkRelationsOfEmployee(employee: Employee): Promise<void> {
        await this.workRelationRepo.update({employeeId: employee.id},
            {isActive: false})
    }

    async getWorkRelationByIds(
        employeeId: number, companyId: number): Promise<WorkRelation> {
        return await this.workRelationRepo.findOne(
